...
- `/why 168` — сводка причин молчания за неделю (`stage:reason count`),
  ...
  `min_replies_between` через `/set`. **`fix:*` (`fix:dash`, `fix:emoji_freq`,
  `fix:emoji_count`) — это не срез, а правка**: реплика ушла в чат такой же,
  только без типографского тире или с поправленным эмодзи (`postprocess.py`,
  CLAUDE.md "Интерфейсы: пост-обработка"). Считать их вместе с `regex:*`/`judge:*`
  для доли резки (раздел «Выключение shadow mode») не нужно. Много `fix:emoji_freq`
  значит модель всё ещё лепит эмодзи в конец почти каждой реплики чаще, чем
  задумано, — повод подкрутить пункт про эмодзи в `prompts/system.txt`, а не
  править `postprocess.py`.
